defmodule SymphonyElixir.CoreTest do
  ...
  test "in-repo WORKFLOW.md renders correctly" do
    ...
    assert prompt =~ "`context-pruner` is the local CLI for bounded file reads"
    assert prompt =~ "Open `.codex/skills/context-pruner/SKILL.md` before discovery work"
    assert prompt =~ "prefer `PRUNER_URL`"
    assert prompt =~ "`JEEVES_PRUNER_URL`"
    assert prompt =~ "http://192.168.1.15:8000/prune"
    assert prompt =~ "Fall back to bounded raw reads only when `context-pruner` is unavailable"
